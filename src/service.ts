...
import { StateStore, defaultStateFile, type DeletedWorktree, type PersistedState } from './state.ts'
import {
...
export class WorktreeSyncService {
  ...
  snapshot(): WorktreeSnapshot {
    ...
    const repos: RepoInfo[] = this.repos.map((repo) => {
      ...
      return {
        ...
        worktrees,
        deletedWorktrees: state.deletedWorktrees
          .filter(entry => entry.repoKey === repo.key)
          .map(({ path, branch, title }) => ({ path, branch, title })),
      }
    })
    for (const [key, failed] of this.failedRepos) {
      repos.push({
        ...
        worktrees: [],
        deletedWorktrees: state.deletedWorktrees
          .filter(entry => entry.repoKey === key)
          .map(({ path, branch, title }) => ({ path, branch, title })),
        error: failed.error,
      })
    }
  ...
  private async apply(plan: SyncPlan, state: PersistedState): Promise<void> {
    ...
    const deletedPaths = new Set(state.deletedWorktrees.map(entry => entry.path))
    const released = new Set(plan.releaseIgnored.filter(path => !deletedPaths.has(path)))
    ...
    // Ordinary native re-registration lifts a legacy ignore. Explicitly deleted
    // worktrees retain their tombstone until the restore endpoint removes it.
    for (const path of [...this.ignoring]) {
      if (registeredPaths.has(path) && !deletedPaths.has(path)) this.ignoring.delete(path)
    }
    ...
    const next: PersistedState = {
      ...
      ignored: state.ignored.filter(path => !released.has(path)
        && (!registeredPaths.has(path) || deletedPaths.has(path))),
    ...
    try {
      switch (endpoint) {
        ...
        case 'setWorkspaceTitle': {
          ...
          if (workspaceId === undefined) return fail('left-panel/bad-request', 'setWorkspaceTitle requires workspaceId')
          const raw = typeof (payload as Record<string, unknown>)?.title === 'string'
            ? (payload as Record<string, string>).title
            : undefined
          const title = raw?.trim()
          if (title === undefined || title === '') return fail('left-panel/bad-request', 'setWorkspaceTitle requires a non-blank title')
          const workspace = this.ctx.workspaceRegistry.get(workspaceId as WorkspaceId)
          if (workspace === undefined) return fail('left-panel/unknown-workspace', `${workspaceId} is not a registered Workspace`, { workspaceId })
          const repo = this.repos.find(candidate => candidate.worktrees.some(worktree => worktree.path === workspace.path))
          if (repo === undefined) return fail('left-panel/not-worktree', `${workspaceId} is not part of a scanned repository`, { workspaceId })
          const repoPaths = new Set(repo.worktrees.map(worktree => worktree.path))
          const duplicate = this.ctx.workspaceRegistry.list().some(candidate => candidate.id !== workspace.id
            && repoPaths.has(candidate.path) && candidate.title === title)
          if (duplicate) return fail('left-panel/name-conflict', `Workspace name '${title}' is already in use in this repository`, { title, repoKey: repo.key })
          if (title !== workspace.title) await workspace.setTitle(title)
          return ok(this.snapshot())
        }
        case 'deleteWorkspace': {
          const workspaceId = stringField(payload, 'workspaceId')
          if (workspaceId === undefined) return fail('left-panel/bad-request', 'deleteWorkspace requires workspaceId')
          const workspace = this.ctx.workspaceRegistry.get(workspaceId as WorkspaceId)
          if (workspace === undefined) return fail('left-panel/unknown-workspace', `${workspaceId} is not a registered Workspace`, { workspaceId })
          const repo = this.repos.find(candidate => candidate.worktrees.some(worktree => worktree.path === workspace.path))
          const worktree = repo?.worktrees.find(candidate => candidate.path === workspace.path)
          if (repo === undefined || worktree === undefined) {
            return fail('left-panel/not-worktree', `${workspaceId} is not part of a scanned repository`, { workspaceId })
          }
          if (worktree.main) return fail('left-panel/main-worktree', 'The main worktree Workspace record cannot be deleted', { workspaceId })
          ...
          await this.store.update(state => ({
            ...state,
            ignored: state.ignored.includes(deleted.path) ? state.ignored : [...state.ignored, deleted.path],
            deletedWorktrees: [...state.deletedWorktrees.filter(entry => entry.path !== deleted.path), deleted],
          }))
          this.ignoring.add(deleted.path)
          this.rememberSelfDelete(workspaceId)
          await this.ctx.workspaceRegistry.delete(workspaceId as WorkspaceId)
          await this.syncNow()
          return ok(this.snapshot())
        }
        case 'restoreWorkspace': {
          const path = stringField(payload, 'path')
          if (path === undefined) return fail('left-panel/bad-request', 'restoreWorkspace requires path')
          const deleted = this.store.current.deletedWorktrees.find(entry => entry.path === path)
          if (deleted === undefined) return fail('left-panel/unknown-deleted-worktree', `${path} is not a deleted worktree`, { path })
          const worktree = this.repos
            .find(repo => repo.key === deleted.repoKey)?.worktrees
            .find(candidate => candidate.path === deleted.path && candidate.exists && !candidate.prunable)
          if (worktree === undefined) {
            return fail('left-panel/worktree-missing', `The worktree directory no longer exists: ${path}`, { path })
          }
          this.ignoring.delete(deleted.path)
          await this.store.update(state => ({
            ...state,
            ignored: state.ignored.filter(candidate => candidate !== deleted.path),
            deletedWorktrees: state.deletedWorktrees.filter(entry => entry.path !== deleted.path),
          }))
          await this.syncNow()
          const workspace = await this.ctx.workspaceRegistry.resolveByPath(deleted.path)
          if (workspace !== undefined && workspace.title !== deleted.title) await workspace.setTitle(deleted.title)
          await this.syncNow()
          return ok(this.snapshot())
        }
        default:
